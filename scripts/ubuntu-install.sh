...
# Installed package version (e.g. 0.2.34), empty if not present.
...
# Start tray daemon + open onboarding/settings so install is not a dead end.
post_install_launch() {
    ...
    if ! run_as_desktop_user bash -c "
        ...
        true
    "; then
        # Fallback: current user (non-root install path)
        if [[ $EUID -ne 0 ]]; then
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --force)
                ...
                ;;
            *)
                # Ignore unknown args so handoff/curl wrappers stay tolerant.
                ;;
        esac
        shift
    done
